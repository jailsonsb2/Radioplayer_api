...
(function () {
  ...
  const getDataFromStreamAfrica = async (artist, title, defaultArt, defaultCover) => {
    ...
    if (title === 'juanjui server' || response.status === 403) {
      const results = {
        title,
        artist,
        art: defaultArt,
    ...
    const results = {
      ...
      thumbnail: stream.artwork?.small || defaultArt,
      art: stream.artwork?.large || defaultArt,
      cover: stream.artwork?.xl || defaultCover,
      stream_url: stream.stream || '#not-found'
    };
    cache[cacheKey] = results;
    return results;
  };
...
function normalizeHistory(api) {
  let artist;
  let song;
  let history = api.song_history || api.history || api.songHistory || api.mscp?.playlist || [];
  
  if (api.mscp?.playlist) {
    history = history.slice(1, 5); // Omitir el primer elemento y limitar a 4 elementos solo para mscp
  } else {
    history = history.slice(0, 4); // Limitar a 4 elementos para las otras APIs
  }
  ...
  function normalizeTitle(api) {
    ...
    } else if (api.mscp && api.mscp.playlist && api.mscp.playlist.length > 0) {
        title = api.mscp.playlist[0].title;
        artist = api.mscp.playlist[0].artist;
...
<a href="{{stream_url}}" class="history-spotify" target="_blank" rel="noopener">
  ...
  function initApp() {
    ...
    function init(current) {
      ...
      const server = currentStation.server || 'spotify';
